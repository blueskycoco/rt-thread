...
static rt_err_t samv71_configure(struct rt_serial_device *serial, struct serial_configure *cfg)
{
	...
	unsigned short parity = US_MR_PAR_NO;
	...
	uart->UartHandle->US_MR
		= (US_MR_USART_MODE_NORMAL | parity | US_MR_USCLKS_MCK
		   | US_MR_CHRL_8_BIT);
	uart->mode = (US_MR_USART_MODE_NORMAL | parity | US_MR_USCLKS_MCK
		   | US_MR_CHRL_8_BIT);
