...
extern void tiva_eth_mac_addr_reconfig(uint8_t *mac_addr);
void rt_hw_led_on();
void rt_hw_led_off();
...
unsigned char config_net_protol0[1+8]			={0xF5,0x8A,0x14,0xff,0x26,0xfa,0x00,0x00};/*ipv4 or ipv6 0*/
unsigned char config_net_protol1[1+8]			={0xF5,0x8A,0x15,0xff,0x26,0xfa,0x00,0x00};/*ipv4 or ipv6 1*/
unsigned char config_net_protol2[1+8]			={0xF5,0x8A,0x16,0xff,0x26,0xfa,0x00,0x00};/*ipv4 or ipv6 2*/
unsigned char config_net_protol3[1+8]			={0xF5,0x8A,0x17,0xff,0x26,0xfa,0x00,0x00};/*ipv4 or ipv6 3*/
unsigned char config_socket_mode0[1+8]			={0xF5,0x8A,0x18,0xff,0x26,0xfa,0x00,0x00};/*client or server 0*/
unsigned char config_socket_mode1[1+8]			={0xF5,0x8A,0x19,0xff,0x26,0xfa,0x00,0x00};/*client or server 1*/
unsigned char config_socket_mode2[1+8]			={0xF5,0x8A,0x1a,0xff,0x26,0xfa,0x00,0x00};/*client or server 2*/
unsigned char config_socket_mode3[1+8]			={0xF5,0x8A,0x1b,0xff,0x26,0xfa,0x00,0x00};/*client or server 3*/
...
unsigned char config_tcp0[1+8]					={0xF5,0x8A,0x21,0xff,0x26,0xfa,0x00,0x00};/*tcp or udp 0*/
...
extern int _epi_write(int index, const void *buffer, int size,unsigned char signal);
int times=0;
extern int _usb_write(int index, void *buffer, int size);
int baud(int type);
...
void set_config(rt_uint8_t *data,int ipv6_len,int dev)//0 no change ,1 local socket need reconfig ,2 all socket need reconfig
{
	...
	switch(data[0])
	{
		...
		case 7:
		{
			...
			rt_uint8_t *tmp=rt_malloc(16);
			rt_memset(tmp,'\0',16);
			rt_sprintf(tmp,"%d.%d.%d.%d.%d.%d",data[1],data[2],data[3],data[4],data[5],data[6]);
			if(memcmp(tmp,g_conf.mac,16)!=0)
			{
				ipv4_changed=true;
				rt_sprintf(g_conf.mac,"%d.%d.%d.%d.%d.%d",data[1],data[2],data[3],data[4],data[5],data[6]);
				//tiva_eth_mac_addr_reconfig(data);
			}
			rt_free(tmp);
		}
		break;
...
void common_w_epi(void* parameter)
{
	...
	while(1)
	{
		if (rt_sem_take(&(rx_sem[0]), RT_WAITING_FOREVER) != RT_EOK) continue;
		_epi_read();
	}	
}
...
void bus_speed_test(void *param)
{
	...
	rt_uint8_t config_ripv6[]={0xf5,0x8a ,0x0c ,0x1a ,0x66 ,0x65 ,0x38 ,0x30 ,0x3a ,0x3a ,0x64 ,0x37 ,0x30 ,0x3a ,0x66 ,0x39 ,0x61
	,0x3a ,0x63 ,0x63 ,0x64 ,0x32 ,0x3a ,0x31 ,0x63 ,0x37 ,0x32 ,0x25 ,0x31 ,0x33 ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff
	,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff 
	,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0xff ,0x26 ,0xfa ,0x2e ,0xa2};
	...
	rtt_kprintf("\n\nset PC ipv6 to fe80::d70:f9a:ccd2:1c72%%13\n\n");
	rt_thread_delay(300);
