...
#define GPRS_STATE_GET_ERROR		20
...
//const uint8_t qistat[] 		= "AT+QISTAT\r\r\n"; m26
const uint8_t qistat[] 		= "AT+QISTATE=0,1\r\r\n";
//const uint8_t qiclose[] 	= "AT+QICLOSE\r\r\n"; m26
const uint8_t qiclose[] 	= "AT+QICLOSE=0\r\r\n";

const uint8_t qilocip[] 	= "AT+QILOCIP\r\r\n";
const uint8_t qilport[] 	= "AT+QILPORT?\r\r\n";
const uint8_t e0[] 			= "ATE0\r\r\n";
const uint8_t cgreg[] 		= "AT+CGREG?\r\r\n";
const uint8_t cimi[] 		= "AT+CIMI\r\r\n";
const uint8_t cpin[] 		= "AT+CPIN?\r\r\n";
const uint8_t qifgcnt[] 	= "AT+QIFGCNT=0\r\r\n";
const uint8_t qisrvc[] 		= "AT+QISRVC=1\r\r\n";
const uint8_t qimux[] 		= "AT+QIMUX=0\r\r\n";
const uint8_t ask_qimux[] 	= "AT+QIMUX?\r\r\n";
//const uint8_t qideact[]		= "AT+QIDEACT\r\r\n"; m26
const uint8_t qideact[]		= "AT+QIDEACT=1\r\r\n";
const uint8_t qindi[]		= "AT+QINDI=1\r\r\n";
//const uint8_t qird[]		= {"AT+QIRD=0,1,0,1500\r\r\n"}; m26
const uint8_t qird[]		= {"AT+QIRD=0,1500\r\r\n"};

const uint8_t qisack[]	= "AT+QISACK\r\r\n";
const uint8_t qiat[]	="AT\r\r\n";
const uint8_t qierror[] = "AT+QIGETERROR\r\r\n";
uint8_t 	  qicsgp[32]	= {0};
uint8_t 	  qiopen[64]	= {0};
uint8_t 	  qisend[32] 	= {0};
const uint8_t qiregapp[]	= "AT+QIREGAPP\r\r\n";
//const uint8_t qiact[] 		= "AT+QIACT\r\r\n"; m26
const uint8_t qiact[] 		= "AT+QIACT=1\r\r\n";
			...
			switch (g_gprs_state) {
				...
				case GPRS_STATE_CHECK_QISTAT:
						if (have_str(last_data_ptr, STR_STAT_INIT) ||
							...
							(have_str(last_data_ptr,STR_TCP) && have_str(last_data_ptr,"uart1"))){
							...
							gprs_at_cmd(qiopen);
						} else if(have_str(last_data_ptr, STR_OK)) { //e20
				...
				case GPRS_STATE_SET_QIACT:
						if (have_str(last_data_ptr, STR_OK)) {
							...
							//g_gprs_state = GPRS_STATE_CHECK_QISTAT; m26
							//gprs_at_cmd(qistat);
							g_gprs_state = GPRS_STATE_SET_QIDEACT;
							gprs_at_cmd(qideact);
						}
				...
				case GPRS_STATE_SET_QIOPEN:
						if (have_str(last_data_ptr, STR_CONNECT_OK) ||
							have_str(last_data_ptr, STR_CONNECT_OK_EC20)) {
							g_gprs_state = GPRS_STATE_DATA_PROCESSING;
							/*send data here */
							...
							if (!have_str(last_data_ptr, STR_OK)) {
								//rt_thread_delay(100*3); m26
								//g_gprs_state = GPRS_STATE_CHECK_QISTAT;
								//gprs_at_cmd(qistat);
								g_gprs_state = GPRS_STATE_GET_ERROR;
								gprs_at_cmd(qierror);
							}
						}
						break;
				case GPRS_STATE_GET_ERROR:
						g_gprs_state = GPRS_STATE_CHECK_QISTAT;
						gprs_at_cmd(qistat);
						break;
