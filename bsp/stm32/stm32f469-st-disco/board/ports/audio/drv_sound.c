...
static rt_err_t  stm32_player_configure(struct rt_audio_device *audio, struct rt_audio_caps *caps)
{
    ...
    case AUDIO_TYPE_MIXER:
    {
        ...
        case AUDIO_MIXER_MUTE:
        {
            ...
            LOG_D("set codec mutes");
            cs43l22_SetMute(_stm32_audio_play.i2c_bus, AUDIO_I2C_ADDRESS,
            		    AUDIO_MUTE_ON);
            break;
        }
        ...
        case AUDIO_MIXER_VOLUME:
        {
            ...
            LOG_D("set codec volume %d", volume);
            cs43l22_SetVolume(_stm32_audio_play.i2c_bus, AUDIO_I2C_ADDRESS,
            		    volume);
...
static rt_err_t stm32_player_start(struct rt_audio_device *audio, int stream)
{
    if (stream == AUDIO_STREAM_REPLAY)
    {
        ...
        cs43l22_Play(_stm32_audio_play.i2c_bus, AUDIO_I2C_ADDRESS);
    }
...
static rt_err_t stm32_player_stop(struct rt_audio_device *audio, int stream)
{
    if (stream == AUDIO_STREAM_REPLAY)
    {
        ...
        cs43l22_Stop(_stm32_audio_play.i2c_bus, AUDIO_I2C_ADDRESS);
    }
